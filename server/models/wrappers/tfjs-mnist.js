...
class datasetWrapper {
    ...
    addExample(example, value, units) {
      const y = this.tf.tidy(() => this.tf.tensor2d(value, [1,units]).toFloat());
  
      if (this.xs == null) {
        this.xs = this.tf.keep(example);
        this.ys = this.tf.keep(y);
      } else {
        const oldX = this.xs;
        this.xs = this.tf.keep(oldX.concat(example, 0));
  
        const oldY = this.ys;
        this.ys = this.tf.keep(oldY.concat(y, 0));
